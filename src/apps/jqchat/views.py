...
def window(request, id):
    ...
    return render_to_response('jqchat/chat_test.html', {'room': ThisRoom,'usuario':request.session['nombres'],'fecha':request.session['login_date'],'dep':request.session['dependencia']}, context_instance=RequestContext(request))
