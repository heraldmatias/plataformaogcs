...
from forms import (ResumenPrensaForm, 
    ...
    CasoExitoForm, ConsultaCeForm, CeTable)
...
#############################################################################################
@login_required()
def add_caso_exito(request, codigo=None):
    obj = None
    if codigo:
        obj = get_object_or_404(CasoExito,pk=codigo)
    if request.method == 'POST':
        if 'archivo' in request.FILES:
            profile = request.user.get_profile()
            ini=request.session['dependencia']
            filename1 = request.FILES['archivo'].name
            ext = filename1[filename1.rfind('.')+1:]
            filename= "CE%s%s.%s" % (ini.iniciales,
                datetime.strptime(request.POST['fecha'],"%d/%m/%Y").strftime("%d%m%Y"),ext)            
            if not obj:
                obj = CasoExito(organismo=profile.organismo,
                    dependencia=profile.dependencia,idusuario_creac_id=profile.numero,)
            else:
                obj.idusuario_mod = profile
                obj.fec_mod = datetime.today()           
            request.FILES['archivo'].name = filename
            filename1 = obj.archivo
        formulario = CasoExitoForm(request.POST, request.FILES, instance= obj, error_class=DivErrorList)        
        if formulario.is_valid():
            if codigo and 'archivo' in request.FILES:
                FileSystemStorage().delete(filename1)
            formulario.save()            
            messages.add_message(request, messages.SUCCESS, 
               'Registro %s exitosamente!' % ((codigo)and'Modificado'or'Grabado') )
            if codigo:
                return redirect('ogcs-mantenimiento-prensa-ce-query')
            formulario = CasoExitoForm()
    else:
        formulario = CasoExitoForm(instance= obj)    
    return render_to_response('prensa/ce.html', {'formulario': formulario,
        'resumen':((codigo) and obj or None)}, context_instance=RequestContext(request),)
...
def print_caso_exito(request):
    ...
    if 'fechaini' in request.GET and 'fechafin' in request.GET:
        ...
        if start_date and end_date:
            ...
            query = query.filter(fecha__lte=end_date)    
    data = query.extra(select={'dependencia':"""case tbl_caso_exito.organismo_id when 1
         then (select ministerio from ministerio where nummin=tbl_caso_exito.dependencia) 
         when 2 then (select odp from odp where numodp=tbl_caso_exito.dependencia) 
         when 3 then (select gobernacion from gobernacion 
            where numgob=tbl_caso_exito.dependencia) end"""}).values('codigo',
    'idusuario_creac__nombres','idusuario_creac__apellidos','organismo__nombre',
    'fecha','dependencia','descripcion','archivo')    
    domain = get_current_site(request).domain
    html = render_to_string('prensa/ce_reporte.html',{'data': data,
        'protocol':request.is_secure() and 'https' or 'http',
        'pagesize':'A4','usuario':request.user.get_profile(),'domain':domain},
        context_instance=RequestContext(request))
    filename= "ce_%s.pdf" % datetime.today().strftime("%Y%m%d")        
    return imprimirToPDF(html,filename) 