...
from forms import UsuarioForm, UsuarioTable, ConsultaUsuarioForm,EditUsuarioForm, AdministradorForm,EditAdministradorForm,AdministradorTable,ConsultaAdministradorForm
from django.template import RequestContext
from usuario.models import Usuario, Estado,Administrador
...
def useredit(request, codigo):
    ...
    return render_to_response('usuario/usuario.html', {'frmusuario': frmusuario,'opcion':'edit','codigo':codigo,'usuario':request.session['nombres'],'fecha':request.session['login_date'],'dependencia':dependencia,'mensaje':mensaje}, context_instance=RequestContext(request),)
...
def adminadd(request):
    if request.method == 'POST':
        num = Administrador.objects.values("numero").order_by("-numero",)[:1]
        num = 1 if len(num)==0 else int(num[0]["numero"])+1
        frmusuario = AdministradorForm(request.POST) # A form bound to the POST data
        if frmusuario.is_valid():
            if request.POST['organismo'] == "1":
                usernamee = Ministerio.objects.get(nummin=request.POST['dependencia']).iniciales+'_'+(serie-len(str(num)))*'0'+str(num)
            elif request.POST['organismo'] == "2":
                usernamee = Odp.objects.get(numodp=request.POST['dependencia']).iniciales+'_'+(serie-len(str(num)))*'0'+str(num)
            elif request.POST['organismo'] == "3":
                usernamee = Gobernacion.objects.get(numgob=request.POST['dependencia']).iniciales+'_'+(serie-len(str(num)))*'0'+str(num)
            user = User.objects.create_user(username=usernamee,email=request.POST['email'],password=request.POST['contrasena'],)
            if request.POST['nivel'] == "2":
                user.is_staff = True
                user.is_superuser = True
            ...
            user.last_name = request.POST['apellidos']
            user.save()
            usuario = Administrador(user=user,numero=num,usuario=usernamee)
            frmusuario = AdministradorForm(request.POST, instance=usuario)
            frmusuario.save()
            usuario.contrasena = user.password
            usuario.save() 
            asunto="Bienvenido a la plataforma de Comunicacion Social"
        ...
        frmusuario = AdministradorForm()
    return render_to_response('usuario/admin.html', {'frmusuario': frmusuario,'opcion':'add','usuario':request.session['nombres'],'fecha':request.session['login_date'],'dependencia': request.POST['dependencia'] if 'dependencia' in request.POST else 0}, context_instance=RequestContext(request),)

@login_required()
def adminedit(request, codigo):
    mensaje=''
    if request.method == 'POST':
        profile = Usuario.objects.get(user = request.user)
        usuario = Usuario.objects.get(numero=int(codigo))
        ...
        usuario = get_object_or_404(Administrador, numero=int(codigo))
        dependencia = usuario.dependencia
        frmusuario = EditAdministradorForm(instance=usuario)
    return render_to_response('usuario/admin.html', {'frmusuario': frmusuario,'opcion':'edit','codigo':codigo,'usuario':request.session['nombres'],'fecha':request.session['login_date'],'dependencia':dependencia,'mensaje':mensaje}, context_instance=RequestContext(request),)
...
def adminquery(request):
    ...
    if "2-sort" in request.GET:
        col = request.GET['2-sort']
    config = RequestConfig(request)
    consultausuarioform = ConsultaAdministradorForm(request.GET)
    filtro = list()
    usuarios = None
    dependencia=0
    if ('nombres' in request.GET and 'organismo' in request.GET and 'estado' in request.GET and 'apellidos' in request.GET) or 'dependencia' in request.GET:
        if request.GET['nombres']:
            usuarios = Administrador.objects.filter(nombres__icontains=request.GET['nombres'])
        if request.GET['organismo']:
            filtro.append(u"organismo_id =%s"%request.GET['organismo'])
        if request.GET['estado']:
            filtro.append(u"estado_id =%s"%request.GET['estado'])
        if request.GET['apellidos']:
            if usuarios:
                ...
                usuarios = Administrador.objects.filter(apellidos__icontains=request.GET['apellidos'])
        if 'dependencia' in request.GET:
            if request.GET['dependencia']:
                filtro.append(u"dependencia =%s"%request.GET['dependencia'])
                dependencia=request.GET['dependencia']
    ...
    filtro.append(u'numero >0')
    if usuarios is None:
        usuarios = Administrador.objects.extra(where=filtro)
    else:
        usuarios = usuarios.extra(where=filtro)
    usuarios = usuarios.extra(select={'dependencia':"case organismo_id when 1 then (select ministerio from ministerio where nummin=dependencia) when 2 then (select odp from odp where numodp=dependencia) when 3 then (select gobernacion from gobernacion where numgob=dependencia) end"})
    tblusuarios = AdministradorTable(usuarios.order_by(col))
    config.configure(tblusuarios)
    tblusuarios.paginate(page=request.GET.get('page', 1), per_page=6)
    return render_to_response('usuario/admin_consulta.html', {'consultausuarioform':consultausuarioform,'tabla':tblusuarios,'usuario':request.session['nombres'],'fecha':request.session['login_date'],'dependencia':dependencia}, context_instance=RequestContext(request),)
...
def adminprint(request):
    ...
    if usuarios is None:
        ...
        usuarios = usuarios.extra(where=filtro)
    usuarios = usuarios.extra(select={'dependencia':"case organismo_id when 1 then (select ministerio from ministerio where nummin=dependencia) when 2 then (select odp from odp where numodp=dependencia) when 3 then (select gobernacion from gobernacion where numgob=dependencia) end"})
    filename= "administrador_%s.xls" % datetime.today().strftime("%Y%m%d")
    return imprimirToExcel('usuario/reporteadmin.html', {'data': usuarios,'fecha':datetime.today().date(),'hora':datetime.today().time(),'usuario':request.session['nombres']},filename)
